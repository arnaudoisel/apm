...
BUILD_WORKFLOW = ROOT / ".github" / "workflows" / "docs-build.yml"
...
def _build_workflow() -> dict:
    return load_workflow(BUILD_WORKFLOW)
...
def test_docs_build_has_read_only_permissions() -> None:
    ...
    assert build_workflow["permissions"] == {"contents": "read"}
    assert workflow_job(build_workflow, "build")["permissions"] == {"contents": "read"}
...
def test_docs_checker_step_mutations_are_rejected(mutation: str) -> None:
    ...
    workflow = deepcopy(_build_workflow())
