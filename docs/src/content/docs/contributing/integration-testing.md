...
- **Routing-cell coverage**: `test_primitive_target_covering_array.py` derives static catalog rows from `KNOWN_TARGETS` (`tests/utils/lifecycle_interactions.py`): every valid primitive/target/scope cell, named target-set transitions in both scopes, and two dynamic refusals. Shared execution clears ambient native-root overrides before fixture writes and CLI setup. Commands observe complete project/user roots. Fixture sources and reviewed layouts authorize writes, not the ledger. Checks bind claims to source owners, require regular native fixture leaves, protect independently authored prose in an unowned neighbor outside generated output, retain foreign native hook/sidecar members, and validate generated Copilot instruction content/ownership against surviving package sources.
- **Source-interaction coverage**: catalog cells combine direct/transitive packages, Git/local sources, compatible ref/cache states, clean/tampered content, and reinstall/audit/update. Credit requires every law and action witness, including tag advancement before tagged updates, warm-cache rematerialization, and applicable cold-cache clearing. Audits require exact `--ci` status and fixture-attributed JSON; unavailable-target refusals require exit 1 plus target-specific diagnosis and recovery guidance. Arbitrary failure earns no credit. `tests/quality/test_lifecycle_interaction_coverage.py` guards generation/accounting without CLI calls; tracked bugs grant no coverage or CI exemption.
- **Source-runtime mutation campaign**: `tests/integration/test_lifecycle_mutation_campaign.py` exercises wrong-target deployment, an omitted ownership claim, and skipped target cleanup against unchanged routing oracles through child-process Python console entry points, not frozen binaries. `lifecycle_mutation` JUnit properties feed separate `--mutation-output` JSON. `--require-mutations` requires all three IDs exactly once, killed after green baselines. Ordered successful commands, fixture-bound effects, exact oracle law/path diagnostics, passing pytest nodes, and unchanged parent owners/catalog/source determine credit; setup errors, timeouts, unreachable probes, failures/skips, and isolation failures cannot earn kills. Wrong-target detection checks missing authorized output after reconciliation, not every transient write. Reports retain rejection reasons and are saved before completeness gates.
- **Command intent**: no-change update does not promise repair; a separate install repairs tampering, followed by convergence checks. Stable targets are manifest-declared; experimental-target audit limitations remain visible. Target contraction uses install in both scopes; companion prune is project-only (`apm prune` has no `--global`).
- **Runtime and scope**: `ApmLifecycleRunner.scenario()` bounds interleaved commands/actions, not hosted runtime. Unfiltered integration includes routing, model, and mutation cases; real cases stay outside required PR smoke, while fast contracts remain there. Routing rows also run in broad release suites, not focused Intel selection. Marketplace/OCI, virtual packages, MCP/LSP-only dependencies, and dynamic-target success are outside this campaign; other suites grant no credit.
- **Report boundary**: CI combines four distinct shard directories and requires every scheduled case's evidence. `--require-complete` permits uncovered pairs explained by the exact known gap below, without granting it successful credit. The reporter consumes only `lifecycle_execution` and `lifecycle_mutation`; neither switch certifies model properties or tests without those properties. Preserve every pytest exit status independently of reporting.
- **Run source-interaction coverage locally (POSIX, repository root)**: see [development setup](../development-guide/#development-environment). This pins the source CLI and preserves pytest's exit status independently:
  ```sh
  (
    uv sync --frozen --extra dev || exit "$?"
    pytest_status=0
    APM_E2E_TESTS=1 APM_BINARY_PATH="$PWD/.venv/bin/apm" \
      uv run --frozen --extra dev --no-sync pytest \
        tests/integration/test_primitive_target_covering_array.py \
        tests/integration/test_lifecycle_mutation_campaign.py \
        --junitxml=lifecycle-junit.xml -o junit_family=legacy || pytest_status=$?
    report_status=0
    uv run --frozen --extra dev --no-sync python -m tests.utils.lifecycle_interaction_report \
      --junit lifecycle-junit.xml --revision "$(git rev-parse HEAD)" \
      --output lifecycle-interactions.json \
      --mutation-output lifecycle-mutations.json --require-mutations --require-complete \
      || report_status=$?
    printf 'pytest=%s report=%s\n' "$pytest_status" "$report_status"
    if [ "$pytest_status" -ne 0 ]; then exit "$pytest_status"; fi
    exit "$report_status"
  )
  ```
- **Final acceptance**: on the final combined production/framework candidate, independently require **514 covered pairs, zero uncovered, all three named triple witnesses, and 123 accepted main routing records plus exactly `copilot-instructions-user`** with reason `copilot-user-instruction-second-pass-convergence`. Inspect raw JUnit for the narrow strict idempotency expected failure, model execution/laws/replay/search, scope-parity controls, and all four warm/cold clean/refusal startup controls; the two-module recipe is not this acceptance bundle. Hosted runtime and frozen-binary validation remain separate.

  :::note[Planned]
  Final startup controls are pending in [#2820](https://github.com/microsoft/apm/pull/2820); this local recipe does not run them.
  :::

- **Evidence identity**: `--revision` labels reports; it does not authenticate source or executables. Require a candidate-specific immutable receipt: production/framework/combined revision pins, invocation-time source/interpreter or binary identity, commands/statuses, and raw artifact paths/digests. Use fresh evidence directories; verify input membership before accepting reports.
- **Known gaps**: late lockfile replacement can leave new-target files with the prior lockfile; required tests bound changes and require next-install convergence. Global Copilot instruction reinstall can duplicate a block before third-pass convergence; safety checks remain mandatory. Transaction expansion and identity unification are separate production decisions. `tests/fixtures/lifecycle_bug_ledger.json` records both.
