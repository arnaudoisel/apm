...
- **Source-runtime mutation campaign**: `tests/integration/test_lifecycle_mutation_campaign.py` exercises wrong-target skill deployment, an omitted serialized ownership claim, and skipped removed-target cleanup, each against a green baseline and unchanged routing oracles. Mutations exist only in child processes running the installed Python console entry point, not the frozen `apm_binary_path`; production sources remain unchanged. The real cases carry `lifecycle_smoke` and `lifecycle_merge_group`, excluding them from required PR smoke. Fast accounting contracts remain in required smoke.
- **Mutation report handoff**: each test emits a dedicated `lifecycle_mutation` JUnit property, not interaction coverage. The reporter's `--mutation-output` writes separate per-shard or combined JSON; `--require-mutations` requires all three IDs exactly once and all three killed. It re-derives outcomes from ordered traces and exact oracle failures, rather than trusting claimed counts. Invocation errors, timeouts, unreachable probes, pytest failures/skips, and failed isolation checks cannot earn kills. The wrong-target mutant is detected through missing authorized output after reconciliation, not observation of every transient write. Reports are saved before completeness gates; this bounded source campaign does not establish exhaustive or frozen-binary mutation coverage.
